	...
	class Box extends HTMLElement {
		...
		render(val, info, color, bgcolor) {
			...
			if(rounded >=0 && rounded <=100) {
				this.$style.innerHTML = ':host {border-radius: 10px;border-width: 2px;border-color: black;border-style: solid;display: block;}.container {background: ' + bgcolor + ';}.metric {padding: 10%;}.metric svg {max-width: 100%;}.metric path {stroke-width: 75;stroke: #ecf0f1;fill: none;}.metric text {font-family: "Lato", "Helvetica Neue", Helvetica, Arial, sans-serif;}.metric.participation path.data-arc {stroke: ' + color + ';}.metric.participation text {fill: ' + color + ';}';
		...
		onCustomWidgetAfterUpdate(changedProperties) {
            ...
            if ("bgcolor" in changedProperties) {
